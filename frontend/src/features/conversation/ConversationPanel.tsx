        ...
        if (!dep || !arr || !dt || (!departureTime && !hasServicePreference)) {
          ...
          if (recs.length === 0) {
            if (!routeExists) {
              ...
              // 직행만 다루므로 그 사실을 정직하게 안내한다. 여기서 세션을 통째로 초기화하면
              // (예전 버그) 그동안 답한 날짜/인원/좌석선호 등 이번 실패와 무관한 정보까지 전부
              // 잃어버린다. 위쪽 1차 노선 체크 실패와 동일하게 같은 sessionId를 유지해, 서버가
              // 살려둔 정보는 그대로 두고 출발지/도착지만 다시 물어보게 한다.
